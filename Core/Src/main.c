...
#define TOGGLE_SWITCH_PIN GPIO_PIN_8
#define TOGGLE_SWITCH_PORT GPIOA
...
int main(void)
{
	...
	while (1){
		if (HAL_GPIO_ReadPin(BUTTON_GPIO_PORT, BUTTON_PIN) == GPIO_PIN_SET) { // Button pressed
			...
			__HAL_TIM_SET_COMPARE(&htim2, TIM_CHANNEL_1, motorSpeed); // Set PWM duty cycle
			}
		else {
			// Stop the motor
       ...
       // LED control logic
       if (HAL_GPIO_ReadPin(TOGGLE_SWITCH_PORT, TOGGLE_SWITCH_PIN) == GPIO_PIN_SET) {
           // Blink LEDs one after another
           HAL_GPIO_WritePin(LED_GPIO_PORT, LED_RED_PIN, GPIO_PIN_SET);
           HAL_Delay(blinkDuration);
